...
(function () {
	...
	// ─── AGENT 2.0 LOGIC ───
	window.adminOpenAgentModal = function () {
		const modal = document.getElementById('modal-agent-generator');
		const rawInput = document.getElementById('input-agent-raw');
		if (rawInput) rawInput.value = '';
		if (modal) modal.style.display = 'flex';
	};

	window.adminCloseAgentModal = function () {
		const modal = document.getElementById('modal-agent-generator');
		if (modal) modal.style.display = 'none';
	};

	window.adminRunAgent2 = async function () {
		const rawInput = document.getElementById('input-agent-raw');
		const loadingStatus = document.getElementById('agent-loading-status');
		const btn = document.getElementById('btn-agent-generate');
		
		if (!rawInput || !rawInput.value.trim()) return;
		...
		try {
			...
			// AI reply should be strict JSON but let's sanitize it just in case
			aiReply = aiReply.replace(/```json/gi, '').replace(/```/g, '').trim();
			
			const chapterData = JSON.parse(aiReply);
			
			const chId = `chapter-ai-${Date.now()}`;
			const chapterObj = {
				id: chId,
				season: 'Season 01',
				tag: chapterData.tag || 'AI Chapter',
				title: chapterData.title || 'Untitled AI Chapter',
				description: chapterData.description || 'Generated by Agent 2.0',
				files: []
			};

			chaptersData.push(chapterObj);
			saveChaptersLocally();
			window.showToast('Agent 2.0 created a new chapter!', 'check');
			window.adminCloseAgentModal();
			
		} catch (error) {
			console.error("Agent 2.0 Error:", error);
			window.showToast('Agent 2.0 failed to parse the data. Try again.', 'close');
		} finally {
